...
## Two limitations, stated upfront

1. **Threshold tuning was compressed, not skipped.** The 5-day spec's Day-4 tuning lives here as an
   automated sweep over `[0.60…0.85]` in `evaluate.py`; the kept value and rationale are in
   `output/threshold_tuning.json` and the report.
2. **Mock-LLM circularity.** `llm_mock.py` heuristics were written by the same author who injected the
   hard cases, so mock accuracy measures heuristic coverage — not model generalization. The transferable
   property is the interface + routing invariant, swappable for a real model behind the same I/O.
